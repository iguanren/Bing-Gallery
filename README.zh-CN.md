...
一个安静的必应每日壁纸画廊 —— 4K、无水印、全量归档。[English](README.md) | **中文简体**
...
![今日必应壁纸](https://cn.bing.com/th?id=OHR.MisurinaPeak_ZH-CN3877105161_1920x1080.jpg)
...
- **整站中英双语** — 导航栏、筛选条、归档、页脚到 About 页全部双语，一键静默切换、不刷新
- **真·英文内容** — 英文模式下，壁纸标题与版权描述取自必应英文市场**同一张图**的数据（按图片指纹匹配），不是机器翻译
...
```
index.html              首页（结构与 SEO 元信息，逻辑都在 JS 文件里）
about.html              About 页
style.css               两页共用的全部样式
site.js                 公共逻辑：语言切换 / 夜间模式 / 悬浮按钮 / 运行天数
gallery.js              首页逻辑：数据加载、卡片流、按月归档、灯箱
about.js                About 页逻辑：今日壁纸、版权描述双语
data.json               全部壁纸数据（自动生成）
scripts/fetch_bing.py   每日抓取 + 链接体检
.github/workflows/      定时自动更新
```
